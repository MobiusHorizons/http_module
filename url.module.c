...
export typedef char * parameter [2];
...
export Url * parse(const char * in, int * error){
  ...
  if (qs != NULL){
    ...
    if (qs && params_count > 0){
      u->params = malloc(sizeof(parameter) * (params_count + 1));
      bool kv = 0;
      char * c = qs;
      int i = 0;
...
export char * query_string(Url * u){
  if (u->qs == NULL) return "";

  return u->qs;
}

export void free(Url * u){
  if (u){
    global.free(u->host_port);
    global.free(u->_mem);
    global.free(u);
  }
}
